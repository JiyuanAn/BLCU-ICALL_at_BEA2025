...
RUBRIC = """
A: Yes (the tutor's response clearly indicates what the student should do next)
B: To some extent (the response demonstrates some awareness of what the student should do next, but is vague, unclear, or easy to misunderstand)
C: No (the response does not provide any details related to what the student should do next)
"""
...
def main():
    ...
    path_input = os.path.join(path_base, "data", "mrbench_v3_devset_train.json")
    path_output = os.path.join(path_base, "src", "SFT", "LLaMA-Factory", "data", "mrbench_v3_devset_train_track4.json")
    # 读取数据文件
