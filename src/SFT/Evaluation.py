    ...
    for item in data:
        for model in item['tutor_responses']:
            ...
            for dimension in item['tutor_responses'][model]['annotation_pred']:
                ...
                label = -1
                if '[RESULT]' in item['tutor_responses'][model]['annotation_pred'][dimension]['response']:
                    text = item['tutor_responses'][model]['annotation_pred'][dimension]['response'].split('[RESULT]')[1].strip()
                    match = re.search(r'[1-3]', text)
                    if match:
                        label = int(match.group()) - 1 
                    else:
                        label = list(ANNOTATION_DICT.values())[-1]
                elif re.search(r'\[[1-3]\]', item['tutor_responses'][model]['annotation_pred'][dimension]['response']):
                    match = re.search(r'[1-3]', item['tutor_responses'][model]['annotation_pred'][dimension]['response'])
                    # print(item['tutor_responses'][model]['annotation_pred'][dimension]['response'])
                    if match:
                        # print(match.group())
                        label = int(match.group()) - 1 
                    else:
...
def main():
    ...
    # 处理数据
    y_true_dict, y_pred_dict = process_data(data, ANNOTATION_DICT)
    classes = CLASSES
