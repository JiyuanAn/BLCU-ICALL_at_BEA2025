        ...
        for model in tqdm(item['tutor_responses'], desc="Evaluating models        ", position=1, leave=False):
            ...
            for evaluation_dimension in tqdm(EVALUATION_DIMENSIONS, desc=f"Evaluating dimensions    ", position=2, leave=False):
                ...
                completion = client.chat.completions.create(
                    model="Llama-8B", #gemini-2.5-pro-preview-03-25
                    messages=[
...
def main():
    ...
    path_output_step = os.path.join(path_base, 'res', 'Llama-8B-V020101_01.jsonl')
    path_output = os.path.join(path_base, 'res', 'Llama-8B-V020101_01.json')
    # 初始化OpenAI客户端
